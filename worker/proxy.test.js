...
describe('/ai/* : cles IA jamais exposees au navigateur', () => {
    ...
    it('refuse (401) sans en-tete Authorization', async () => {
        const res = await aiCall('/ai/key', {
            env: AI_ENV(),
            body: { provider: 'anthropic', key: 'sk-ant-1234567' },
        });
            ...
            routeFetch([
                ...
                [
                    ...
                    (_u, opts) => {
                        ...
                        return {
                            ok: true,
                            json: async () => ({ content: [{ type: 'text', text: 'RESUME IA' }] }),
                        };
                    },
                ],
            ])
                ...
                [
                    ...
                    (_u, opts) => {
                        ...
                        return {
                            ok: true,
                            json: async () => ({
                                content: [{ type: 'text', text: 'ANALYSE REDIGEE' }],
                            }),
                        };
                    },
                ],
    ...
    it('POST /ai/stock-analysis : sert le cache du jour sans rappeler le fournisseur', async () => {
        ...
        await aiCall('/ai/stock-analysis', {
            env,
            headers: { Authorization: 'Bearer jwt-xyz' },
            body,
        });
        const res2 = await aiCall('/ai/stock-analysis', {
            env,
            headers: { Authorization: 'Bearer jwt-xyz' },
            body,
        });
    ...
    it('POST /ai/stock-analysis : force regenere une fois, puis 429 dans l heure', async () => {
        ...
        const res1 = await aiCall('/ai/stock-analysis', {
            env,
            headers: { Authorization: 'Bearer jwt-xyz' },
            body,
        });
    ...
    it('401 si le JWT est rejete par Supabase', async () => {
        ...
        const res = await aiCall('/ai/insights', {
            ...
            body: { provider: 'anthropic', prompt: 'x' },
        });
        expect(res.status).toBe(401);
    });
...
describe('/quote', () => {
    ...
    it('renvoie le prix courant depuis regularMarketPrice', async () => {
        fetchMock.mockResolvedValue(
            jsonFetchResponse(
                chartResult({ symbol: 'AAPL', regularMarketPrice: 225.5, currency: 'USD' })
            )
        );
        const res = await call('/quote?symbol=AAPL');
        expect(res.status).toBe(200);
    ...
    it('renvoie 502 si le symbole est introuvable', async () => {
        fetchMock.mockResolvedValue(
            jsonFetchResponse({ chart: { result: null, error: { description: 'No data found' } } })
        );
...
describe('/search', () => {
    it('filtre les resultats sans symbole et normalise les champs', async () => {
        ...
        expect(await res.json()).toEqual([
            { displaySymbol: 'AAPL', description: 'Apple Inc.', type: 'EQUITY' },
        ]);
    });
});
...
describe('/websearch', () => {
    ...
    it('sert le resultat en cache sans rappeler Tavily', async () => {
        const results = [
            { title: 'Cached', url: 'https://example.com', content: 'x', publishedDate: null },
        ];
...
describe('/quoteSummary', () => {
    ...
    it('normalise la reponse Yahoo v10 (raw extrait, champs a plat)', async () => {
                    ...
                    result: [
                        {
                            price: {
                                longName: 'Microsoft',
                                currency: 'USD',
                                regularMarketPrice: { raw: 420.5 },
                                regularMarketPreviousClose: { raw: 415 },
                                marketCap: { raw: 3.1e12 },
                            },
                            defaultKeyStatistics: {
                                forwardPE: { raw: 32 },
                                pegRatio: { raw: 2.1 },
                                enterpriseToEbitda: { raw: 24 },
                                heldPercentInstitutions: { raw: 0.73 },
                                shortPercentOfFloat: { raw: 0.006 },
                            },
                            financialData: {
                                targetMeanPrice: { raw: 480 },
                                numberOfAnalystOpinions: { raw: 45 },
                                grossMargins: { raw: 0.68 },
                                recommendationKey: 'buy',
                            },
                            summaryDetail: {
                                dividendYield: { raw: 0.0072 },
                                payoutRatio: { raw: 0.25 },
                                fiftyTwoWeekHigh: { raw: 470 },
                                averageVolume: { raw: 22000000 },
                                exDividendDate: { raw: 1755129600 },
                            },
                            recommendationTrend: {
                                trend: [
                                    {
        ...
        expect(body.recommendationTrend).toEqual({
            strongBuy: 20,
            buy: 15,
            hold: 8,
            sell: 1,
...
describe('/fmp', () => {
    ...
    it('appelle le bon endpoint FMP et met en cache 24h', async () => {
        fetchMock.mockResolvedValue(
            jsonFetchResponse([{ calendarYear: '2023', priceEarningsRatio: 34 }])
        );
        const res = await call('/fmp?symbol=msft&resource=ratios', { FMP_API_KEY: 'secret' });
        expect(res.status).toBe(200);
        const url = fetchMock.mock.calls[0][0];
        expect(url).toContain(
            'https://financialmodelingprep.com/api/v3/ratios/MSFT?period=annual&limit=6'
        );
...
describe('/recommendation /insider /peers (Finnhub)', () => {
    ...
    it('transmet la reponse Finnhub pour /recommendation', async () => {
        ...
        expect(fetchMock.mock.calls[0][0]).toContain(
            'finnhub.io/api/v1/stock/recommendation?symbol=AAPL&token=key'
        );
