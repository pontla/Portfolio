...
describe('series de cours simulees (repli sans historique reel)', () => {
    ...
    it('getDailyHistory marque la serie quand le proxy ne renvoie rien', async () => {
        ...
        globalThis.fetch = /** @type {any} */ (async () => ({ ok: true, status: 200, json: async () => ({}) }));
        const h = await A.getDailyHistory('ZZZZ', new Date('2025-01-01'), new Date('2025-03-01'), 100, 110);
        ...
        A.candleCache = {};
        globalThis.fetch = /** @type {any} */ (async () => ({ ok: true, status: 200, json: async () => ({ '2025-01-02': 10, '2025-01-03': 11 }) }));
        const reel = await A.getDailyHistory('ZZZZ', new Date('2025-01-01'), new Date('2025-03-01'), 100, 110);
