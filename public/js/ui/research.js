...
export const research = {
    ...
    initResearch() {
        const input = /** @type {HTMLInputElement} */ (
            document.getElementById('researchSearchInput')
        );
        ...
        input.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                const sym = pick
                    ? pick.displaySymbol || pick.symbol
                    : input.value.trim().toUpperCase();
                if (sym) {
                    closeSuggest();
                    this.runResearch(sym);
                }
        ...
        suggest.addEventListener('mousedown', (e) => {
            const row = /** @type {HTMLElement} */ (
                /** @type {Element} */ (e.target).closest('.rs-row')
            );
            if (row) {
                closeSuggest();
                this.runResearch(row.dataset.sym);
            }
        });

        document.getElementById('researchQuick').addEventListener('click', (e) => {
            const btn = /** @type {HTMLElement} */ (
                /** @type {Element} */ (e.target).closest('button[data-sym]')
            );
            if (btn) this.runResearch(btn.dataset.sym);
        });

        /** @type {NodeListOf<HTMLElement>} */ (
            document.getElementById('researchRange').querySelectorAll('.range-btn')
        ).forEach((btn) => {
            btn.addEventListener('click', () => {
                /** @type {NodeListOf<HTMLElement>} */ (
                    document.querySelectorAll('#researchRange .range-btn')
        ...
        if (maLegend) {
            maLegend.addEventListener('click', (e) => {
                const btn = /** @type {HTMLElement} */ (
                    /** @type {Element} */ (e.target).closest('.ma-toggle')
                );
        ...
        if (addBtn)
            addBtn.onclick = () => {
                (
                    ...
                    document.getElementById('addTransactionFab')
                )?.click();
                setTimeout(() => {
                    const si = /** @type {HTMLInputElement} */ (
                        document.getElementById('symbolInputField')
                    );
                    if (si && this.researchSymbol) {
                        si.value = this.researchSymbol;
    ...
    goToResearch(symbol) {
        ...
        /** @type {HTMLElement} */ (
            document.querySelector('.tab-btn[data-tab="research"]')
        )?.click();
    ...
    onResearchTabShown() {
        ...
        if (this.researchSymbol) {
            if (this.researchChart) this.researchChart.resize();
            return;
        }
    ...
    renderResearchQuick() {
        ...
        if (!wrap) return;
        const stats = this.service.calculatePortfolio(this.chartState.currency);
        const syms = (stats.holdings || [])
            .slice()
            .sort((a, b) => b.valueUSD - a.valueUSD)
            .slice(0, 6)
            .map((h) => h.symbol);
        wrap.innerHTML = syms
            .map((s) => `<button type="button" data-sym="${s}">${s}</button>`)
            .join('');
    },
    ...
    async runResearch(symbol) {
        ...
        /** @type {HTMLImageElement} */ (document.getElementById('researchLogo')).src =
            this.getLogoUrl(symbol);
        ...
        const [fund, name] = await Promise.all([
            ...
            this.assetNameCache[symbol] !== undefined
                ? Promise.resolve(this.assetNameCache[symbol])
                : this.fetchAssetName(symbol),
        ]);
        ...
        document.getElementById('researchMeta').textContent = [fund && fund.exchange, cur]
            .filter(Boolean)
            .join(' · ');
    ...
    async runDeepAnalysis() {
        ...
        try {
            ...
            if (a) {
                this.applyResearchAnalysis(symbol, a);
                Icons.render();
    ...
    renderResearchMaLegend(available, colors) {
        ...
        /** @type {NodeListOf<HTMLElement>} */ (box.querySelectorAll('.ma-toggle')).forEach(
            (btn) => {
                const key = btn.dataset.ma;
    ...
    renderResearchPosition(symbol, cur, price) {
        ...
        if (this.service.activePortfolioId === 'GLOBAL' && h.portfolios && h.portfolios.length) {
            ptfEl.textContent = h.portfolios
                .map((id) => (this.service.getPortfolioById(id) || {}).name)
                .filter(Boolean)
                .join(', ');
        } else ptfEl.textContent = '';

        const kv = (k, v, cls = '') =>
            `<div class="research-kv"><span class="k">${k}</span><span class="v ${cls}">${v}</span></div>`;
        ...
        document.getElementById('researchPositionGrid').innerHTML =
            ...
            kv(
                '+/- value latente',
                `${h.gainNative >= 0 ? '+' : ''}${Utils.formatCurrency(h.gainNative, cur)} (${Utils.formatPercent(h.gainPercent)})`,
                gainCls
            ) +
            kv('Poids portefeuille', Utils.formatPercent(h.weightPercent, false)) +
            kv('Dividendes reçus', Utils.formatCurrency(dividends, cur)) +
            kv(
                'P&L réalisé (hors frais)',
                `${realized >= 0 ? '+' : ''}${Utils.formatCurrency(realized, cur)}`,
    ...
    renderResearchKey(fund, cur, price) {
        ...
        if (!fund) {
            grid.innerHTML = `<div class="research-kv"><span class="v">Données indisponibles.</span></div>`;
            src.textContent = '';
            bar.hidden = true;
            return;
        }
        ...
        grid.innerHTML =
            ...
            kv(
                'Croissance CA (1 an)',
                fund.revenueGrowthTTM == null ? '—' : Utils.formatPercent(fund.revenueGrowthTTM)
            ) +
            kv('Volume du jour', Utils.formatCompact(fund.volume)) +
            kv(
                'Clôture veille',
                fund.previousClose == null ? '—' : Utils.formatCurrency(fund.previousClose, cur)
            );

        const lo = fund.fiftyTwoWeekLow,
            hi = fund.fiftyTwoWeekHigh;
    ...
    renderResearchAbout(fund) {
        ...
        if (fund && fund.weburl) {
            ...
            try {
                host = new URL(fund.weburl).hostname.replace(/^www\./, '');
            } catch (e) {}
            kv(
                'Site',
                `<a href="${fund.weburl}" target="_blank" rel="noopener noreferrer">${host}</a>`
            );
        }
        if (!rows.length) {
            card.hidden = true;
    ...
    async renderResearchChart(symbol) {
        ...
        if (this.researchChart) {
            ...
            this.researchChart.options.plugins.tooltip.callbacks.label = (ctx) =>
                this._researchTip(ctx, cur);
            this.researchChart.options.scales.y.ticks.callback = (v) =>
                Utils.formatCurrency(v, cur);
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        mode: 'index',
                        intersect: false,
                        callbacks: { label: (ctx) => this._researchTip(ctx, cur) },
                    },
                },
                scales: {
                    x: {
                        ...
                        ticks: { color: ink.tick, font: { size: 11 }, maxTicksLimit: 7 },
                    },
                    y: {
                        position: 'right',
                        ...
                        ticks: {
                            color: ink.tick,
                            font: { size: 11 },
                            callback: (v) => Utils.formatCurrency(v, cur),
                        },
                    },
                },
            },
