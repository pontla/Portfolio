...
export const researchFundamentals = {
    renderResearchValuation(a) {
        ...
        const cmp = (cur, avg) => {
            if (cur == null || avg == null || !isFinite(cur) || !isFinite(avg) || avg <= 0)
                return '';
            const above = cur > avg;
            return (
                `<span class="kv-cmp ${above ? 'above' : 'below'}">${above ? '▲' : '▼'} ` +
        ...
        grid.innerHTML =
            ...
            kv(
                'Rendement FCF',
                v.fcfYield == null ? null : Utils.formatPercent(v.fcfYield, false),
    ...
    _growthSeries(title, points, fmt, tip) {
        ...
        const rows = points
            .map((p, i) => {
                ...
                return (
                    `<div class="gs-row"><span class="gs-year">${p.year || '—'}</span>` +
    ...
    renderResearchGrowth(a) {
        ...
        const lastRevYear =
            Number((AnalysisUtils.arr(g.revenueAnnual).slice(-1)[0] || {}).year) || 0;
        const nextEst =
            estAnnual.find((e) => Number(e.year) > lastRevYear) ||
            estAnnual[estAnnual.length - 1] ||
            null;
        const yahooY1 =
            AnalysisUtils.arr(g.estimatesShortTerm).find((e) => e.period === '+1y') || null;
        const estRev =
            nextEst && nextEst.revenueAvg != null
                ? nextEst.revenueAvg
                : yahooY1
                  ? yahooY1.revenueAvg
                  : null;
        const estEps =
            nextEst && nextEst.epsAvg != null ? nextEst.epsAvg : yahooY1 ? yahooY1.epsAvg : null;
        const estYear =
            (nextEst && nextEst.year) ||
            (yahooY1 && yahooY1.endDate ? String(yahooY1.endDate).slice(0, 4) : null);
        const analysts =
            g.analystCount != null
                ? g.analystCount
                : nextEst && nextEst.analysts != null
                  ? nextEst.analysts
                  : yahooY1
                    ? yahooY1.analysts
                    : null;
    ...
    renderResearchHealth(a) {
        ...
        const netDebt =
            h.totalDebt != null && h.totalCash != null ? h.totalDebt - h.totalCash : null;

        grid.innerHTML =
            kv(
                'Dette nette / EBITDA',
                mult(h.netDebtToEbitda),
            ...
            kv(
                'Trésorerie',
                h.totalCash == null ? null : money(h.totalCash),
                'Trésorerie et placements court terme au dernier bilan publié.'
            ) +
            kv(
                'Dette totale',
                h.totalDebt == null ? null : money(h.totalDebt),
                'Dettes financières court et long terme au dernier bilan publié.'
            ) +
            kv(
                'Dette nette',
                netDebt == null ? null : money(netDebt),
                'Dette totale moins la trésorerie. Négative : la société a plus de cash que de dettes.',
                netDebt == null
                    ? ''
                    : `<span class="kv-tag ${netDebt <= 0 ? 'ok' : 'mid'}">${netDebt <= 0 ? 'trésorerie nette' : 'endettée'}</span>`
            );

        const trendLabel =
            { croissant: 'en hausse', stable: 'stable', décroissant: 'en baisse' }[h.fcfTrend] ||
            null;
    ...
    _sparkRow(label, points, tip) {
        ...
        return (
            `<div class="spark-row">${head}${this._sparkline(pts)}` +
    ...
    renderResearchProfitability(a) {
        ...
        grid.innerHTML =
            kv(
                'ROE',
                prof.roeReliable ? pct(p.roe) : p.roe == null ? null : NS,
                "Résultat net rapporté aux capitaux propres : ce que la société génère avec l'argent des actionnaires. Au-dessus de 15 % durablement, c'est solide." +
                    (prof.roeReliable
                        ? ''
                        : " Ici les fonds propres sont négatifs : le ratio change de signe et n'est plus interprétable."),
            ...
            kv(
                'ROIC',
                prof.roicReliable ? pct(p.roic) : p.roic == null ? null : NS,
                "Rentabilité du capital réellement investi (dette + fonds propres). S'il dépasse durablement le coût du capital (~8-10 %), la société crée de la valeur." +
                    (prof.roicReliable
                        ? ''
                        : " Ici le capital investi est négatif : le ratio n'est plus interprétable."),
